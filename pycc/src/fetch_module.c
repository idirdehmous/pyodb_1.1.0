...
// odbFetchnum : numeric-only fetch  ---->  returns numpy.ndarray
//
static PyObject * odbFetchnum_method(PyObject *Py_UNUSED(self), PyObject *args, PyObject *kwargs)
{
    import_array();
...
/*if (lheader) {
    // steals references
     PyList_Insert( result , ( Py_ssize_t ) 0, header_list ); 
     PyList_Insert( result , ( Py_ssize_t ) 1, array  );
} 
else {
    // decref header list  
    //Py_DECREF(header_list);
    PyList_Insert(result, ( Py_ssize_t ) 0, Py_None);     // create new ref  
    PyList_Insert(result, ( Py_ssize_t ) 1, array  );
}
